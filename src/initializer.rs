...
use crate::sim_control::{SimBuilder, SimulationControl};
use crate::skylink_drone::drone::SkyLinkDrone;

pub fn initialize(file: &str) -> (Arc<RwLock<SimBuilder>>, Vec<JoinHandle<()>>) {
    ...
    let builder = SimBuilder::new(network_graph, VecDeque::new());
    let pass_builder = Arc::new(RwLock::new(builder));
    let mut sim_contr = SimulationControl::new(command_send, event_recv, event_send, packet_senders, pass_builder.clone());
    handles.push(thread::spawn(move || {
        sim_contr.run();
